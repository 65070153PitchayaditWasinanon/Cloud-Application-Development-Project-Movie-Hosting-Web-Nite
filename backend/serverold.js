...
uploadRouter.post("/upload", upload.single("videoFile"), async (req, res) => {
  try {
    ...
    const s3Url = `https://${process.env.S3_BUCKET_RAW}.s3.${process.env.AWS_REGION}.amazonaws.com/${key}`;
    ...
    res.json({
      ok: true,
      message: `อัปโหลด "${title}" สำเร็จ`,
      videoUrl: s3Url,
      meta: { title, description, imagePath, rentalPrice },
    });
  } catch (error) {
    console.error("Upload error:", error);
    res
      .status(500)
      .json({ message: "เกิดข้อผิดพลาดในการอัปโหลดไฟล์", error: error.message });
  }
});
